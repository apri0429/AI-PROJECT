...
const DESCRIPTION_BRANDS = [
  { key: "all", label: "SEMUA", match: "", available: true },
  { key: "gosave", label: "GOSAVE", match: "gosave", available: true },
  { key: "goto", label: "GOTO", match: "goto", available: true },
  { key: "safety_bro", label: "SAFETY BRO", match: "gosave", available: true },
];
...
function getBatchStatusText(status) {
  if (status === "generating") return "AI sedang menyusun deskripsi";
  ...
  return (
    <span className="doc-history-date">
      <span className="doc-history-date-pill">
        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" aria-hidden="true">
          <rect x="4.75" y="5.75" width="14.5" height="13.5" rx="2.25" stroke="currentColor" strokeWidth="1.65" />
          <path
...
function GenerateTab() {
  const [brand, setBrand] = useState("goto");
  ...
  const selectedBrand = DESCRIPTION_BRANDS.find((option) => option.key === brand);
  ...
  const handleGenerate = async () => {
    ...
    if (names.length === 0 || !selectedBrand?.available) return;
    ...
    for (const name of names) {
      ...
      try {
        const effectiveBrand =
          brand === "all"
            ? detectBrand(products.find((p) => p.product_name === name) || {})
            : brand;
        const result = await generateDescription(name, effectiveBrand);
        setBatchResults((prev) =>
          prev.map((item) =>
  ...
  return (
    <>
        <p className="im-subtitle">
          Pilih brand dan produk (bisa lebih dari satu), deskripsi marketplace-nya akan otomatis
          dibuat sebagai Google Doc baru.
        </p>

        <div className="brand-choice-panel description-brand-panel">
          ...
          <div className="brand-choice-options" role="radiogroup" aria-label="Pilih brand description">
            {DESCRIPTION_BRANDS.map((option) => (
              <button
                key={option.key}
                type="button"
                role="radio"
                ...
                className={"brand-choice-option" + (brand === option.key ? " active" : "")}
                onClick={() => handleSelectBrand(option.key)}
                disabled={generating}
              >
                {option.label}
                {!option.available && <span className="brand-choice-soon">Segera hadir</span>}
              </button>
            ))}
          </div>
        </div>

        {!selectedBrand?.available ? (
          <div className="im-empty-state">
            <span>Template deskripsi untuk brand ini belum tersedia.</span>
          </div>
        ) : (
          <>
              ...
              <div className={"batch-results" + (generating ? " is-generating" : " is-complete")}>
                <div className="batch-results-header">
                  ...
                  <div className="batch-results-title">
                    <span>{generating ? "Membuat deskripsi" : "Deskripsi selesai"}</span>
                    <small>
                      {generating
                        ? "AI sedang menyusun deskripsi dan menyimpannya ke Drive."
            ...
            <div className="im-actions">
              <button
                type="button"
                className={"im-generate-btn" + (generating ? " is-loading" : "")}
              ...
              >
                {generating ? (
                  ...
                  <>
                    ...
                    <span>Generate Description{selectionCount > 0 ? ` (${selectionCount})` : ""}</span>
                  </>
                )}
              </button>
            </div>
...
function HistoryTab() {
  ...
  const load = () => {
    ...
    fetchDescriptionHistory()
      ...
      .catch((err) => setError(err.message || "History deskripsi belum bisa dimuat. Coba restart server backend."))
  ...
  const handleDeleteDoc = async (doc) => {
    ...
    setError(null);
    try {
      await deleteDescriptionHistoryItem(doc.id);
  ...
  return (
    <>
      <p className="im-subtitle">Semua deskripsi yang pernah dibuat, diambil langsung dari folder history di Google Drive.</p>
        ...
        {folderUrl && (
          <a href={folderUrl} target="_blank" rel="noreferrer" className="history-folder-link">
            <svg className="google-drive-icon" viewBox="0 0 24 24" width="18" height="18" fill="none" aria-hidden="true">
              <path d="M8.55 3.75h6.9l5.55 9.62-3.45 5.98H6.45L3 13.37 8.55 3.75Z" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round" />
              <path d="M8.55 3.75 12 9.73m3.45-5.98-3.45 5.98m0 0-5.55 9.62M12 9.73l2.1 3.64H21M9.9 13.37H21" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.72" />
            </svg>
            <span>Buka folder di Drive</span>
          </a>
        )}
        <button className="history-refresh-btn" onClick={load} disabled={loading} title={loading ? "Memuat history" : "Refresh history"} aria-label={loading ? "Memuat history" : "Refresh history"}>
          {loading ? (
            <>
              <span className="button-spinner" />
              <span>Loading</span>
            ...
            <>
              <svg viewBox="0 0 24 24" width="15" height="15" fill="none" aria-hidden="true">
                <path d="M20 11a8 8 0 0 0-14.6-4.5M4 5v4h4M4 13a8 8 0 0 0 14.6 4.5M20 19v-4h-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <span>Refresh</span>
            </>
          )}
      ...
      <div className="im-search-box history-search-box">
        ...
        <input className="im-search-input" type="text" placeholder="Cari history deskripsi..." value={search} onChange={(event) => setSearch(event.target.value)} disabled={loading} />
      </div>
      ...
      <div className="doc-history-list">
          {!loading && !error && docs.length === 0 && (
            <div className="im-empty-state">
              <span>Belum ada deskripsi yang dibuat.</span>
            </div>
          )}
          ...
          {filteredDocs.map((doc) => (
            <div key={doc.id || doc.url} className="doc-history-item">
              <a className="doc-history-main" href={doc.url} target="_blank" rel="noreferrer">
                ...
                <CalendarDate value={formatDate(doc.modified_at)} />
              </a>
              <div className="doc-history-actions">
                <CopyLinkButton url={doc.url} copied={copiedId === (doc.id || doc.url)} onCopy={() => handleCopyLink(doc)} />
                <button type="button" className="doc-history-delete-btn" onClick={() => setConfirmDeleteId(doc.id)} disabled={deletingId === doc.id} title="Hapus history" aria-label="Hapus history">
                  {deletingId === doc.id ? (
                    <span className="doc-delete-dot" />
                  ) : (
                    <>
                      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" aria-hidden="true">
                        <path d="M3.75 6.25h16.5M9 6.25V4.8c0-.58.47-1.05 1.05-1.05h3.9c.58 0 1.05.47 1.05 1.05v1.45m3 0-.63 12c-.05.84-.74 1.5-1.58 1.5H8.21c-.84 0-1.53-.66-1.58-1.5L6 6.25m4.25 4.5v5.5m3.5-5.5v5.5" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                      <span>Hapus</span>
                    </>
                  )}
...
export default function DescriptionPage({ onToggleSidebar }) {
